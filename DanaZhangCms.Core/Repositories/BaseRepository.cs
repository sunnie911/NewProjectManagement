...
namespace DanaZhangCms.Core.Repositories
{
    public abstract class BaseRepository<T, TKey> : IRepository<T, TKey> where T : BaseModel<TKey>
    {
        ...
        public virtual async Task<T> GetSingleAsync(TKey key, Func<IQueryable<T>, IQueryable<T>> include)
        {
            if (include == null) return await DbContext.FindAsync<T, TKey>(key);

            var filter = DbContext.FilterWithInclude(include, o => true);

            return await filter.FirstOrDefaultAsync<T>(o => o.Id.Equal(key));
        }
        /// <summary>
        ...
        public virtual IQueryable<T> GetByPaginationWithInclude(Expression<Func<T, bool>> @where, Func<IQueryable<T>, IQueryable<T>> include, int pageSize, int pageIndex, bool asc = true, params Func<T, object>[] @orderby)
        {
            var filter = DbContext.FilterWithInclude(include, where);
            if (orderby != null)
            {
                foreach (var func in orderby)
                {
